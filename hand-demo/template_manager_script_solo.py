...
def send_result(result):
    ...
    ${_TRACE2} (f"Sending result: {result}")
    buffer = buffer_mgr(len(result_serial))   

    buffer.setData(json.dumps(result).encode('utf-8'))
...
def send_result_no_hand(pd_inf, nb_lm_inf):
    # result = dict([("pd_inf", pd_inf), ("nb_lm_inf", nb_lm_inf), ("lm_score", 0), ("handedness", 0), ("rotation", 0),
    #         ("rect_center_x", 0), ("rect_center_y", 0), ("rect_size", 0), 
    #         ("rrn_lms", 0), ('sqn_lms', 0), ('world_lms', 0), ("xyz", 0), ("xyz_zone", 0)])
...
def send_result_hand(pd_inf, nb_lm_inf, lm_score=0, handedness=0, rect_center_x=0, rect_center_y=0, rect_size=0, rotation=0, rrn_lms=0, sqn_lms=0, world_lms=0, xyz=0, xyz_zone=0):
    ...
    # result = [pd_inf, nb_lm_inf, lm_score, handedness, rect_center_x, rect_center_y, rect_size, rotation, rrn_lms, sqn_lms, world_lms, xyz, xyz_zone]
    send_result(result)
