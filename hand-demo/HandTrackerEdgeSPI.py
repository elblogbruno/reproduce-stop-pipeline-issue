...
class HandTracker:
    ...
    """
    def __init__(self, flash=False, input_src=None,
        ...
        if input_src == None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            self.laconic = True # input_src == "rgb_laconic" # Camera frames are not sent to the host
            if resolution == "full":
        ...
        if self.flash:
            ...
            bootloader = dai.DeviceBootloader(bl)
            progress = lambda p : print(f'Flashing progress: {p*100:.1f}%')
            bootloader.flash(progress, pipeline, False)
        else:
            print("Device already flashed")
            self.device.startPipeline(pipeline)
            print(f"Pipeline started - USB speed: {str(usb_speed).split('.')[-1]}")

            while not self.device.isClosed():
                time.sleep(1)
    ...
    def create_pipeline(self):
        ...
        spiIn.setBusId(0)
        spiIn.out.link(manager_script.inputs['spimetain'])
        ...
        manager_script.outputs['host'].link(spiOut.input)
        manager_script.outputs['camControl'].link(cam.inputControl)
        
        self.cfg = dai.CameraControl()
    ...
    def stop_camera(self):
        self.ctrlQ = self.device.getInputQueue(name="camControl")
        self.cfg.setStopStreaming()
        self.ctrlQ.send(self.cfg)

    def start_camera(self):
        self.ctrlQ = self.device.getInputQueue(name="camControl")
        self.cfg.setStartStreaming()
        self.ctrlQ.send(self.cfg)
