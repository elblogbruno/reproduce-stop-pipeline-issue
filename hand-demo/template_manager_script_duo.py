...
def send_result_no_hand(pd_inf, nb_lm_inf):
    # ${_TRACE1} (f"send_result_no_hand: pd_inf={1 if pd_inf else 0}, nb_lm_inf={nb_lm_inf}")
    result = dict([("pd_inf", pd_inf), ("nb_lm_inf", nb_lm_inf)])
    # result = ":"+str(pd_inf)+":"+str(nb_lm_inf)+":"
    send_result(result)
...
def send_result_hands(pd_inf, nb_lm_inf, lm_score, handedness, rect_center_x, rect_center_y, rect_size, rotation, rrn_lms, sqn_lms, world_lms, xyz, xyz_zone):
    ...
    result = dict([("pd_inf", pd_inf), ("nb_lm_inf", nb_lm_inf), ("lm_score", lm_score), ("handedness", handedness), ("rotation", rotation),
            ("rect_center_x", rect_center_x), ("rect_center_y", rect_center_y), ("rect_size", rect_size), ("rrn_lms", rrn_lms), ("xyz", xyz)])
    # result = ":"+str(pd_inf)+":"+str(nb_lm_inf)+":"+str(lm_score)+":"+str(handedness)+":"+str(rotation)+":"+str(rect_center_x)+":"+str(rect_center_y)+":"+str(rect_size)+":"+str(rrn_lms)+":"+str(sqn_lms)+":"+str(world_lms)+":"+str(xyz)+":"+str(xyz_zone)+":"
    # ${_TRACE1} (f"lm_score: {lm_score}")
    # result = [len(lm_score), lm_score, handedness, rect_center_x, rect_center_y, rect_size, rotation, rrn_lms, sqn_lms, world_lms, xyz, xyz_zone]
    send_result(result)
...
cfg_cam_control = CameraControl()
calc = False

data = dict()

#TODO Fix the synchronization between the oak and esp32
while not data: 
...
${_TRACE1} (f"Esp32 is alive: {msg_dict}")


while True:
    data = node.io['spimetain'].get().getData()
    ${_TRACE1} (str(data, 'utf-8'))
